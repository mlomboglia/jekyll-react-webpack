...
gulp.task("server", ["build"], () => {
  ...
  gulp.watch(gulpConfig.styles.watch, ["styles"]).on("error", err => {
    log(err, err.toString(), ["Styles"])
    this.emit(end)
  })
  gulp.watch(gulpConfig.scripts.watch, ["scripts"]).on("error", err => {
    log(err, err.toString(), ["Scripts"])
    this.emit(end)
  })
  gulp.watch(gulpConfig.svg.watch, ["svg"]).on("error", err => {
